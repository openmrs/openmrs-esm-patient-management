...
import { type MappedUnscheduledAppointment } from '../hooks/useUnscheduledAppointments';
...
export function exportUnscheduledAppointmentsToSpreadsheet(
  unscheduledAppointments: Array<MappedUnscheduledAppointment>,
...
function createWorksheet(data: Array<Record<string, string | number>>) {
  const max_width = data.reduce((w, r) => Math.max(w, (r['Patient name'] as string).length), 30);
