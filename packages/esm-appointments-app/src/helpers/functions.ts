...
export const isValidPhoneNumber = (value: string): boolean => {
  ...
  if (cleanValue.length < 7 || cleanValue.length > 15) return false;
  if (!/^\+?\d+$/.test(cleanValue)) return false;
  if (cleanValue.toLowerCase() === 'false' || cleanValue.toLowerCase() === 'true') return false;
  if (/^0+$/.test(cleanValue) && cleanValue.length >= 7) return false;
  if (/^1+$/.test(cleanValue) && cleanValue.length >= 7) return false;
  return true;
};
