...
describe('DailyCalendarView', () => {
  ...
  it('shows empty state banner and range bars without empty hour rows when no appointments', async () => {
  ...
  it('renders in normal state by default showing active appointments across ranges while hiding empty hours', async () => {
    mockOpenmrsFetch.mockResolvedValue({
      data: [
        appointmentAt('07:30', 'early-appt'),
        appointmentAt('10:00', 'live-appt'),
        appointmentAt('18:00', 'late-appt'),
      ],
    } as FetchResponse);
    ...
    await waitFor(() => {
      expect(screen.getByTestId('hour-row-7')).toBeInTheDocument();
      expect(screen.getByTestId('hour-row-10')).toBeInTheDocument();
      expect(screen.getByTestId('hour-row-18')).toBeInTheDocument();
    });

    expect(screen.queryByTestId('hour-row-8')).not.toBeInTheDocument();
