...
describe('buildHourSlots', () => {
  ...
  it('counts spill-in appointments toward peak and includes them as continuation blocks', () => {
    const a = at('08:55', 'a');
    a.endDateTime = new Date('2026-06-09T09:10:00').getTime();
    const b = at('09:00', 'b');
    const slots = buildHourSlots([a, b]);
    const slot9 = slots.find((s) => s.hour === 9);
    expect(slot9?.blocks.map((bl) => bl.appointment.uuid)).toEqual(['a', 'b']);
    const aBlock = slot9?.blocks.find((bl) => bl.appointment.uuid === 'a');
    expect(aBlock?.isContinuation).toBe(true);
    expect(aBlock?.topPx).toBe(0);
    expect(slot9?.allAppointments.map((appt) => appt.uuid)).toEqual(['a', 'b']);
    expect(slot9?.peak).toBe(2);
  });

  it('surfaces an appointment crossing the clinic opening boundary (7:45-8:15) in hour 8', () => {
    const boundaryAppt = atDur('07:45', 'boundary-1', 30);
    const slots = buildHourSlots([boundaryAppt]);

    const slot7 = slots.find((s) => s.hour === 7);
    const slot8 = slots.find((s) => s.hour === 8);

    expect(slot7?.blocks).toHaveLength(1);
    expect(slot7?.blocks[0].isContinuation).toBe(false);

    expect(slot8?.blocks).toHaveLength(1);
    const hour8Block = slot8?.blocks[0];
    expect(hour8Block?.appointment.uuid).toBe('boundary-1');
    expect(hour8Block?.isContinuation).toBe(true);
    expect(hour8Block?.topPx).toBe(0);
    expect(hour8Block?.heightPx).toBe(30);
  });
