...
const waitingUuid = config.concepts.defaultStatusConceptUuid;
const attendingUuid = config.concepts.defaultTransitionStatus;

const NOW = new Date('2026-08-16T10:00:00.000Z');

function queue(uuid: string, display: string): Queue {
  return { uuid, display, name: display } as Queue;
}
...
describe('useClinicQueueMetrics', () => {
  ...
  it('scopes the request to the given location and to unfinished entries', () => {
    givenData([], []);

    renderHook(() => useClinicQueueMetrics('loc-1'));

    expect(mockUseQueueEntries.mock.calls[0][0]).toEqual({ location: 'loc-1', isEnded: false });
  });
