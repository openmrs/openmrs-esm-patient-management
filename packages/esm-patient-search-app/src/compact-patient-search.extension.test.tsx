...
jest.mock('./compact-patient-search/patient-search.component', () => {
  return jest.fn(({ query, patientClickSideEffect, onPatientSelected }) => (
    <div data-testid="mock-patient-search">
      <button onClick={() => onPatientSelected?.('patient-123')} data-testid="select-action-btn">
        Select Action
      </button>
...
describe('CompactPatientSearchComponent', () => {
  ...
  it('calls onPatientSelected when provided and patient is selected', async () => {
    const onPatientSelected = jest.fn();
    const user = userEvent.setup();
    render(<CompactPatientSearchComponent initialSearchTerm="John" onPatientSelected={onPatientSelected} />);

    const selectBtn = screen.getByTestId('select-action-btn');
    await user.click(selectBtn);

    expect(onPatientSelected).toHaveBeenCalledWith('patient-123');
  });
