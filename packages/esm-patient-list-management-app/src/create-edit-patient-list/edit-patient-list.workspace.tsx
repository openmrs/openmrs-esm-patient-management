...
import { editPatientList, extractErrorMessagesFromResponse, type ErrorObject } from '../api/api-remote';
...
export interface EditPatientListWorkspaceProps {
  patientListDetails: OpenmrsCohort;
  onSuccess?: () => void;
}

const EditPatientListWorkspace: React.FC<EditPatientListWorkspaceProps & DefaultWorkspaceProps> = ({
  patientListDetails,
  onSuccess = () => {},
  closeWorkspace,
}) => {
  ...
  }, [closeWorkspace, cohortDetails, onSuccess, patientListDetails?.uuid, t, validateForm]);
    ...
    <div className={styles.container}>
      ...
      <ButtonSet className={styles.buttonsGroup}>
        ...
        <Button className={styles.button} onClick={handleSubmit} size="xl" disabled={isSubmitting}>
          {isSubmitting ? t('submitting', 'Submitting') : t('editList', 'Edit list')}
        </Button>
      </ButtonSet>
    </div>
  );
...
export default EditPatientListWorkspace;
