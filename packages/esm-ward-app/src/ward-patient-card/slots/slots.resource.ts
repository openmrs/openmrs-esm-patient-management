...
import { ConfigObject, SlotDefinition, SlotElementDefinition } from "../../config-schema";
...
export function useCardSlots(location: string, status: string) {
  ...
  const {wardPatientCardConfig} = useConfig<ConfigObject>();
...
function getSlotFromDefinition(slotDef: SlotDefinition, slotElementsMap: Map<string, WardPatientCardSlotElement>) : WardPatientCardSlot {
